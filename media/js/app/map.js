(function(jQuery) {
    ...
    var Educator = Backbone.Model.extend({
    ...
    var EducatorList = Backbone.Collection.extend({
        model: Educator,
        urlRoot: '/_main/api/v1/educator/',
        queryParams: '',
    ...
    window.EducatorPinView = Backbone.View.extend({
        ...
        render: function() {
            ...
            for (var i = 0; i < this.parentView.mapMarkers.length; i++) {
                var pos = this.parentView.mapMarkers[i].getPosition();

                //if a marker already exists in the same position as this marker
                if (this.latlng.equals(pos)) {
                    //update the position of the coincident marker
                    // by applying a small multipler to its coordinates
                    var newLat = this.latlng.lat() + (
                        Math.random() - 0.5) / 500;
                    var newLng = this.latlng.lng() + (
                        Math.random() - 0.5) / 500;
    ...
    window.EducatorMapView = Backbone.View.extend({
        ...
        getVisibleViewport: function() {
            ...
            return {height: viewportheight -
                        (30 + document.getElementById('site-header')
                                  .clientHeight +
                         document.getElementById('primarynav').clientHeight +
                         document.getElementById('section-thumbnail')
                                  .clientHeight),
                     width: viewportwidth};
        },
        render: function() {
        ...
        renderPopup: function(educator, marker) {
            ...
            jQuery('div.educator-infowindow').parent()
                .css('overflow', 'hidden');
        },
