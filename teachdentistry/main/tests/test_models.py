...
class TeachDentistryReportTest(TestCase):
    ...
    def test_metadata(self):
        ...
        # username
        self.assertEquals(rows.next()[1], 'date_joined')
    ...
    def test_values(self):
        ...
        row = rows.next()
        self.assertEquals(row[0], self.profile.user.date_joined)
        self.assertEquals(row[1], self.profile.user.pk)
        self.assertEquals(row[2], 'M')
        self.assertEquals(row[3], 'S1')
        self.assertEquals(row[4], '')
        self.assertEquals(row[5], '')
        self.assertEquals(row[6], 'coding')
        self.assertEquals(row[7], 'NY')
        self.assertEquals(row[8], 2000)

        self.assertEquals(row[9], 'Other')
        self.assertEquals(row[10], '10027')
        self.assertEquals(row[11], 'A3')
        self.assertEquals(row[12], 'A3')
        self.assertEquals(row[13], 'A3')
        self.assertEquals(row[14], 'A3')
        self.assertEquals(row[15], 'E2')
        self.assertEquals(row[16], 'R6')
        self.assertEquals(row[17], 'G3')
        self.assertEquals(row[18], 'D3')
        self.assertEquals(row[19], True)
